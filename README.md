# 👋 Hi, I'm Sahil Khan

🚀 Software Developer with ~1.3 years of industry experience, currently working at **Tudip Technologies**.  
I focus on building backend services, APIs, and full-stack web applications, with hands-on experience in **healthcare** and **CRM** systems.

I enjoy working on real-world problems, learning by building, and exploring AI-powered developer tooling.

---

## 🧑‍💻 What I Do
- Build and maintain backend services using **Node.js**, **Express**, and **PostgreSQL**
- Work on **REST APIs**, background jobs, caching, and integrations
- Contribute to **healthcare platforms** and **CRM systems (Microsoft Dynamics 365)**
- Build **personal projects** around AI, developer tools, and system design
- Solve DSA problems and continuously improve fundamentals

---

## 🛠 Tech Stack

### Languages
![JavaScript](https://img.shields.io/badge/JavaScript-323330?style=for-the-badge&logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript)
![Python](https://img.shields.io/badge/Python-3670A0?style=for-the-badge&logo=python)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus)

### Backend & APIs
![NodeJS](https://img.shields.io/badge/Node.js-6DA55F?style=for-the-badge&logo=node.js)
![Express.js](https://img.shields.io/badge/Express.js-404d59?style=for-the-badge&logo=express)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql)
![MongoDB](https://img.shields.io/badge/MongoDB-4ea94b?style=for-the-badge&logo=mongodb)
![Redis](https://img.shields.io/badge/Redis-D92B21?style=for-the-badge&logo=redis)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman)

### Frontend
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-38B2AC?style=for-the-badge&logo=tailwind-css)

### DevOps & Tools
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux)

---

## 📌 Featured Projects

### 🔹 AI Code Explainer (Chrome Extension)
- Built a Chrome extension that explains code snippets using an LLM (Hugging Face API)
- Implemented a lightweight backend for request handling and response formatting
- Designed a simple React UI for better developer experience

### 🔹 ACP-Based Coding Agent (TypeScript, Bun)
- Implemented a coding agent using the Agent Client Protocol (ACP)
- Built streaming NDJSON handling, tool-call approval flows, and session persistence
- Developed a terminal-based UI with file and shell operations

---

## 📊 GitHub Stats: ![](https://github-readme-stats.vercel.app/api?username=sahilkhan786000&theme=city_light&hide_border=false&include_all_commits=true&count_private=true)<br/> ![](https://github-readme-streak-stats.herokuapp.com/?user=sahilkhan786000&theme=city_light&hide_border=false)<br/> ![](https://github-readme-stats.vercel.app/api/top-langs/?username=sahilkhan786000&theme=city_light&hide_border=false&include_all_commits=true&count_private=true&layout=compact) 
---

## 🏆 Achievements
- Ranked **Top 3% globally (78/2922)** in CodeStudio contest
- Solved **500+ DSA problems** across LeetCode, GFG, and CodeStudio

---

## 🌐 Connect With Me
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin)](https://linkedin.com/in/sahil-khan-31a45a219)
[![GitHub](https://img.shields.io/badge/GitHub-121013?style=for-the-badge&logo=github)](https://github.com/sahilkhan786000)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=firefox)](your-portfolio-link)

---

⭐ Always open to learning, building, and collaborating on meaningful software.
